...
public class GedcomParser {
	...
	/** logger. **/
	private final static Logger LOGGER = Logger.getLogger(GedcomParser.class.getName());
	...
	public static void main(String[] args) {
		...
		try {
			...
			InputStream gedcomDataStream = config.getInputStream();
			FileFormat fileFormat = Enum.valueOf(FileFormat.class, config.getFileFormat());
			Generatable generator = fileFormat.getInstance();
	...
	private static void initLog() {
		try {
			InputStream inputStream = GedcomParser.class.getResourceAsStream(LOG_PROPERTIES_CONFIG);
